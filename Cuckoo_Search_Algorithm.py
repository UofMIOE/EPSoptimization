...
class CuckooSearch(BaseAlgorithm):
    ...
    def search(self, n=100, iteration=50, pa=0.25,
                 nest=100):
        """
...
#        super(CuckooSearch, self).__init__(function_wrapper, number_of_variables, objective)
...
#        self.__nests = np.array(self.__nests)
        ...
        self.__agents = np.random.uniform(function_wrapper.minimum_decision_variable_values()[0], function_wrapper.maximum_decision_variable_values()[0], (n, dimension))
        self.__nests = np.random.uniform(function_wrapper.minimum_decision_variable_values()[0], function_wrapper.maximum_decision_variable_values()[0], (nest, dimension))
        Pbest = self.__nests[np.array([function_wrapper.objective_function_value(x)
        ...
        for t in range(iteration):
            ...
            for i in self.__agents:
                ...
                if function_wrapper.objective_function_value(i) < function_wrapper.objective_function_value(self.__nests[val]):
                    self.__nests[val] = i

            fnests = [(function_wrapper.objective_function_value(self.__nests[i]), i) for i in range(nest)]
            fnests.sort()
            fcuckoos = [(function_wrapper.objective_function_value(self.__agents[i]), i) for i in range(n)]
            ...
            for i in worst_nests:
                if random() < pa:
                    self.__nests[i] = np.random.uniform(function_wrapper.minimum_decision_variable_values()[0], function_wrapper.maximum_decision_variable_values()[0], (1, dimension))
#                    oneNest = list()
...
#                        self.__nests[i][j] = function_wrapper.maximum_decision_variable_values()[i]
            ...
            self.__nests = np.clip(self.__nests, function_wrapper.minimum_decision_variable_values()[0], function_wrapper.maximum_decision_variable_values()[0])
...
#                        self.__agents[i][j] = function_wrapper.maximum_decision_variable_values()[i]
            self.__agents = np.clip(self.__agents, function_wrapper.minimum_decision_variable_values()[0], function_wrapper.maximum_decision_variable_values()[0])
            self._points(self.__agents)
            self.__nest()
            ...
            if function_wrapper.objective_function_value(Pbest) < function_wrapper.objective_function_value(Gbest):
                Gbest = Pbest

        self._set_Gbest(Gbest)

        return {"best_decision_variable_values": self.get_Gbest(), "best_objective_function_value":self.function_wrapper.objective(self.get_Gbest())}

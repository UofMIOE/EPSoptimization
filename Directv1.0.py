...
class Direct(BaseAlgorithm):
    ...
    def search(self, iterations = 10):
       ...
       if self.objective == 'minimization':
           ...
           # pass-in: x_L = [0], x_U = [250] -> x_L = [-250], x_U = [0]
           x_L = np.matrix([x * -1 for x in (self.function_wrapper.maximum_decision_variable_values())])
           x_U = np.matrix([x * -1 for x in (self.function_wrapper.minimum_decision_variable_values())])
      ...
      # transform C to original search space
       ...
       if self.objective == 'minimization':
           ...
           # if objective = "maximization" evaluate -f(x)
           # max f(x) -> -min -f(x)
       ...
       for t in range(1,iterations + 1): 
           ...
           if self.objective == 'minimization':
              ...
              f_min_max = f_min
               ...
               decision_variable_values = [-1*CC[idx3]]
               f_min_max = -1*f_min
            
           print( "\nbest_decision_variable_values"+repr(decision_variable_values)) 
           
           print("\nbest_objective_function_value"+repr(f_min_max))
